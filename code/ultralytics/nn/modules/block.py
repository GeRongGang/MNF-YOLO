	...
	def __init__(self, dim_in, dim_out, norm_in=True, has_skip=True, exp_ratio=1.0,
		...
		if self.attn_s:
			...
			if v_proj:
				...
				self.v = nn.Identity()
		self.conv_local = SRF(dim_mid, dim_mid, dw_ks, stride=stride, groups=(dim_mid // 16))
		self.se = CFAM(dim_mid, reduction=se_ratio) if se_ratio > 0.0 else nn.Identity()
...
class conv11(nn.Module):
    def __init__(self, a, b, c, d, e):  # yaml中给的三个参数
        """Initializes the ResNetLayer given arguments."""
        super().__init__() 
